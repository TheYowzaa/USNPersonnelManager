    ...
    namespace USNPersonnelManager
    {
        ...
        private string FormatDate(DateTime? date)
        {
            ...
            var totalMonths = (now.Year - date.Value.Year) * 12 + (now.Month - date.Value.Month);

            if (now.Day < date.Value.Day)
                totalMonths--;

            string timeAgo;
            if (totalMonths <= 0)
            {
                var daysAgo = (now - date.Value).Days;
                timeAgo = $"{daysAgo} day{(daysAgo == 1 ? "" : "s")} ago";
            }
            else
            {
                timeAgo = $"{totalMonths} month{(totalMonths == 1 ? "" : "s")} ago";
            }

            return $"{date.Value:dd-MM-yyyy} ({timeAgo})";
        }
        ...
        public class PersonnelInfo
        {
            ...
            public List<string> SPDMemberships { get; set; } = new();
